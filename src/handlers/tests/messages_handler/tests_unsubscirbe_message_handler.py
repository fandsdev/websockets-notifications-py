...
def test_unsubscribe_handler_return_success_response(unsubscribe_handler, unsubscribe_message):
    unsubscribe_response = unsubscribe_handler(unsubscribe_message)

    assert unsubscribe_response.message_type == "SuccessResponse"
    assert unsubscribe_response.incoming_message == unsubscribe_message


def test_unsubscribe_handler_call_storage_unsubscriber_under_the_hood(unsubscribe_handler, unsubscribe_message, mocker, storage, ws_subscribed):
    spy_storage_unsubscriber = mocker.spy(StorageUserUnsubscriber, "__call__")

    unsubscribe_handler(unsubscribe_message)
