...
def test_subscribe_handler_return_success_response(subscribe_handler, subscribe_message):
    subscribe_response = subscribe_handler(subscribe_message)

    assert subscribe_response.message_type == "SuccessResponse"
    assert subscribe_response.incoming_message == subscribe_message


def test_subscribe_handler_call_storage_subscriber_under_the_hood(subscribe_handler, subscribe_message, mocker, storage, ws_registered):
    spy_storage_subscriber = mocker.spy(StorageUserSubscriber, "__call__")

    subscribe_handler(subscribe_message)
